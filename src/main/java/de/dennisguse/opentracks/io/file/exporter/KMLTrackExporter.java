...
public class KMLTrackExporter implements TrackExporter {
    ...
    private void writeMarkers(Track track) throws InterruptedException {
        ...
        try (MarkerIterator markerIterator = contentProviderUtils.getMarkerIterator(track.id())) {
            while (markerIterator.hasNext()) {
                ...
                writeMarker(markerIterator.next(), track.zoneOffset());
            }
        }
        if (hasMarkers) {
            writeEndMarkers();
    ...
    private void writeBeginMarkers(Track track) {
        ...
        printWriter.println("<name>" + StringUtils.formatCData(context.getString(R.string.track_markers, track.name())) + "</name>");
        printWriter.println("<open>1</open>");
    }

    private void writeMarker(Marker marker, ZoneOffset zoneOffset) {
        boolean existsPhoto = MarkerUtils.buildInternalPhotoFile(context, marker.trackId(), marker.photoUrl()) != null;
        if (exportPhotos && existsPhoto) {
            writePhotoOverlay(marker, zoneOffset);
        } else {
            writePlacemark(marker.name(), marker.typeLocalized(), marker.description(), marker.position(), marker.position().time(), zoneOffset);
        }
    }
    ...
    private void writeBeginTrack(Track track) {
        ...
        printWriter.println("<name>" + StringUtils.formatCData(track.name()) + "</name>");
        printWriter.println("<description>" + StringUtils.formatCData(track.description()) + "</description>");
        printWriter.println("<opentracks:trackid>" + track.uuid() + "</opentracks:trackid>");

        printWriter.println("<styleUrl>#" + TRACK_STYLE + "</styleUrl>");
        writeActivityType(track.activityType());
        writeTypeLocalized(track.activityTypeLocalized());
    ...
    private void writePhotoOverlay(Marker marker, ZoneOffset zoneOffset) {
        ...
        printWriter.println("<description>" + StringUtils.formatCData(marker.description()) + "</description>");
        printWriter.print("<Camera>");
        printWriter.print("<longitude>" + marker.position().longitude() + "</longitude>");
        printWriter.print("<latitude>" + marker.position().latitude() + "</latitude>");
        printWriter.print("<altitude>20</altitude>");
        printWriter.print("<tilt>90</tilt>");
        printWriter.println("</Camera>");
        printWriter.println("<TimeStamp><when>" + getTime(zoneOffset, marker.position().time()) + "</when></TimeStamp>");
        printWriter.println("<styleUrl>#" + MARKER_STYLE + "</styleUrl>");
        writeTypeLocalized(marker.typeLocalized());
        ...
        printWriter.println("<Point>");
        printWriter.println("<coordinates>" + getCoordinates(marker.position(), ",") + "</coordinates>");
        printWriter.println("</Point>");
        printWriter.println("</PhotoOverlay>");
    }
